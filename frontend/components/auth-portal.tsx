...
export function AuthScreen() {
  ...
  return <div className="shell py-12 md:py-20"><div className="mx-auto grid max-w-5xl gap-10 lg:grid-cols-[1.1fr_.9fr] lg:items-center">
    <section>
      ...
      <h1 className="mt-3 text-3xl font-bold tracking-tight sm:text-4xl md:text-6xl">Speak in your own words. Keep care connected.</h1>
...
export function CarePortal() {
  ...
  return <div className="shell py-6 sm:py-9 md:py-12">
    <div className="flex flex-col gap-4 sm:flex-row sm:flex-wrap sm:items-start sm:justify-between"><div><p className="eyebrow">{roleLabels[profile.role]} workspace</p><h1 className="mt-2 text-3xl font-bold sm:text-4xl">Hello, {profile.display_name}</h1><p className="mt-2 text-base text-[#5d7078] sm:text-lg">{heading}</p></div><div className="mobile-actions flex gap-3 sm:flex-wrap"><button className="btn btn-secondary" onClick={() => setWorkspaceVersion((current) => current + 1)}>Refresh updates</button><button className="btn btn-secondary" onClick={() => void supabase().auth.signOut()}>Sign out</button></div></div>
